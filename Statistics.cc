...
{
    relation[string(relName)].numTuples=numTuples;
...
{	
	...
	relation[string(newName)].numTuples=relation[oldRelation].numTuples;
    for(auto iter:relation[oldRelation].attSet)
        relation[newRelation].attSet[newRelation+"."+iter.first]=iter.second;
    estimate[newRelation]=relation[newRelation].numTuples;
}
		...
		while(pOr!=NULL){
				...
				if (pComp->code == 1 || pComp->code == 2){
                        ...
                        for(auto & iter:relation)
							if(iter.second.attSet.count(oper1)!=0)
								iter.second.attSet[oper1] /= 3;
       ...
       if(orFactPartial){
        ...
        for(auto &ch:diff)
            for(auto &rel:relation)
				if(rel.second.attSet.count(ch.first)){
					rel.second.attSet[ch.first] = ch.second;
				}
        pA = pA->rightAnd;
    }
...
std::ostream& operator<<(std::ostream& os, const structure1& relation){
    os<<relation.numTuples<<" "<<relation.attSet.size()<<" ";
    for(auto i:relation.attSet){
        os<<i.first<<" "<<i.second<<" ";
    }
    return os;
}

std::istream& operator>>(std::istream& is, structure1& Rel){
    int size;
    is>>Rel.numTuples>>size;
...
void Statistics::LoadAllStatistics() {
		...
		AddRel(relName[0],10000);     
	    AddRel(relName[1],800000);    
	    AddRel(relName[2],6001215);   
	    AddRel(relName[3],1500000);   
	    AddRel(relName[4],150000);    
	    AddRel(relName[5],25);       
	    AddRel(relName[6], 200000);   
	    AddRel(relName[7], 5);        
